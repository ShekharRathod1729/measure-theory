...
\begin{proof}
We first show that $\lambda(\varnothing) = 0$. Set $A_j = \varnothing$ for all $j \geq 1$ in (P4). Then, $\lambda(\varnothing) = \sum \left( \bigsqcup_{j \geq 1} \varnothing \right) = \sum_{j \geq 1} \lambda(\varnothing)$, which is possible only if $\lambda(\varnothing) = 0$.
