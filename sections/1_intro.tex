\section{Introduction -- The Problem of Measure}
...
Thus, we must accept that, with the Axiom of Choice, it is not possible to associate such a ``measure'' satisfying (P1) - (P4) to all possible subsets of $\R$. Instead, we must try to find the ``largest'' class of subsets of $\R$ for which it is possible to assign such a measure. 